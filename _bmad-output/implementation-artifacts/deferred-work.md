...
- D1: `Promise.race` with `expect()` calls at `e2e/tests/debug-session.spec.ts:262` (install+retry section). Whichever expect wins resolves the race; the loser keeps polling and can throw after the test moves on, surfacing as a stray rejection in CI logs. Pre-existing in commit a8348f4 — not introduced by this WIP. Rewrite to `Promise.any` with proper cancellation when the spec gets its next pass.
- D2: `url.includes(/report)` in `mockRunDetailRoutes` at `e2e/tests/debug-session.spec.ts:84` greedily matches any future `/runs/<id>/report-events`, `/reports/*`, etc. — silent fulfill with `{ report_id: null }`. Pre-existing; tighten to a path-segment check when a new `/report*` route lands.
- D3: Recorder produced a `verified_unique: false` selector as `active_candidate_index=0` in `recording.rbs.json` (cmd `e937a5ab89a3`, `text="Musik" >> nth=0`). The `_with_nth_match` last-resort fallback is shipping as the default selector with no review marker in the .robot file. Recorder UX concern — surface a "needs review" tag on the .robot line when active candidate is not verified-unique.
- D4: Robot file mixes `xpath=` (escaped to avoid RF kwarg parsing per fix in 385a38c) and bare `text="…"` (no `text=` prefix discipline). Inconsistency is recorder-side; consumers downstream don't care, but the emitter should standardise to either form-or-the-other for emitted text candidates.
...
- D6: `heal-toggle.spec.ts` baseline: 2 of 8 tests are red on the dev backend (HEAL-VENDORED auto-seed 120s timeout + HEAL-1 FlowEditor `Click` node never renders). Both pre-date this WIP. The FlowEditor failure is the more interesting one — fixture setup likely doesn't seed the .robot file the test expects. Investigate when next touching the FlowEditor flow.
