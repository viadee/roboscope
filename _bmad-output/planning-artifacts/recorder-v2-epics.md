...
- **2026-06-25** — Story **D-5** opened to finish Epic D on the Windows host: native event-hook wiring (`_desktop_loop`) capturing click / double-click / type / combobox / menu / window-focus into the existing translator + selector synthesis + `RPA.Windows` emit. Architecture open-question #3 (`RPA.Windows` vs. raw `pywinauto`) resolved: **`pywinauto` capture + `RPA.Windows` emit now, `platynui` kept as a pluggable Phase-2 emit backend** — see `recorder-desktop-architecture.md`. Story spec: `_bmad-output/implementation-artifacts/d-5-windows-native-event-hook-wiring.md`.
