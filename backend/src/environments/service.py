...
async def list_environments(db: AsyncSession) -> list[Environment]:
    """List all environments."""
    result = await db.execute(select(Environment).order_by(Environment.name))
    return list(result.scalars().all())


async def get_environment(db: AsyncSession, env_id: int) -> Environment | None:
    """Get an environment by ID."""
    result = await db.execute(select(Environment).where(Environment.id == env_id))
    return result.scalar_one_or_none()


async def create_environment(db: AsyncSession, data: EnvCreate, user_id: int) -> Environment:
...
async def update_environment(db: AsyncSession, env: Environment, data: EnvUpdate) -> Environment:
...
async def delete_environment(db: AsyncSession, env: Environment) -> None:
...
async def clone_environment(db: AsyncSession, env: Environment, new_name: str, user_id: int) -> Environment:
    """Clone an environment with a new name."""
    new_env = await create_environment(
        db,
        EnvCreate(
            name=new_name,
    ...
    # Copy packages
    packages = await list_packages(db, env.id)
    for pkg in packages:
    ...
    # Copy variables
    variables = await list_variables(db, env.id)
...
async def list_packages(db: AsyncSession, env_id: int) -> list[EnvironmentPackage]:
...
async def add_package(db: AsyncSession, env_id: int, data: PackageCreate) -> EnvironmentPackage:
...
async def remove_package(db: AsyncSession, env_id: int, package_name: str) -> None:
...
async def list_variables(db: AsyncSession, env_id: int) -> list[EnvironmentVariable]:
...
async def add_variable(db: AsyncSession, env_id: int, data: EnvVarCreate) -> EnvironmentVariable:
...
async def _unset_defaults(db: AsyncSession) -> None:
...
async def search_pypi(query: str) -> list[dict]:
