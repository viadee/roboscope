...
@pytest.fixture
def repo(db_session: Session, user):
...
@pytest.fixture
def second_repo(db_session: Session, user):
...
class TestCreateRun:
    def test_create_run_defaults(self, db_session, user, repo):
        data = _run_create(repo.id)
        run = create_run(db_session, data, user.id)
    ...
    def test_create_run_with_all_fields(self, db_session, user, repo):
        data = _run_create(
            repo.id,
...
class TestGetRun:
    def test_get_run_found(self, db_session, user, repo):
        data = _run_create(repo.id)
        created = create_run(db_session, data, user.id)

        fetched = get_run(db_session, created.id)
        assert fetched is not None
        assert fetched.id == created.id
        assert fetched.target_path == created.target_path

    def test_get_run_not_found(self, db_session):
        fetched = get_run(db_session, 99999)
        assert fetched is None


class TestListRuns:
    def test_list_runs_empty(self, db_session, repo):
        runs, total = list_runs(db_session)
        assert runs == []
        assert total == 0

    def test_list_runs_with_runs(self, db_session, user, repo):
        for i in range(3):
            create_run(db_session, _run_create(repo.id, target_path=f"tests/t{i}"), user.id)

        runs, total = list_runs(db_session)
        assert total == 3
        assert len(runs) == 3

    def test_list_runs_pagination(self, db_session, user, repo):
        for i in range(5):
            create_run(db_session, _run_create(repo.id, target_path=f"tests/p{i}"), user.id)

        # Page 1
        runs_p1, total = list_runs(db_session, page=1, page_size=2)
        assert total == 5
        assert len(runs_p1) == 2

        # Page 2
        runs_p2, _ = list_runs(db_session, page=2, page_size=2)
        assert len(runs_p2) == 2

        # Page 3 (last, partial)
        runs_p3, _ = list_runs(db_session, page=3, page_size=2)
    ...
    def test_list_runs_filter_by_status(self, db_session, user, repo):
        run1 = create_run(db_session, _run_create(repo.id), user.id)
        run2 = create_run(db_session, _run_create(repo.id), user.id)
        update_run_status(db_session, run2, RunStatus.RUNNING)

        runs, total = list_runs(db_session, status=RunStatus.PENDING)
        assert total == 1
        assert runs[0].id == run1.id

        runs, total = list_runs(db_session, status=RunStatus.RUNNING)
        assert total == 1
        assert runs[0].id == run2.id

    def test_list_runs_filter_by_repository_id(self, db_session, user, repo, second_repo):
        create_run(db_session, _run_create(repo.id), user.id)
        create_run(db_session, _run_create(second_repo.id), user.id)

        runs, total = list_runs(db_session, repository_id=repo.id)
        assert total == 1
        assert runs[0].repository_id == repo.id

        runs, total = list_runs(db_session, repository_id=second_repo.id)
        assert total == 1
        assert runs[0].repository_id == second_repo.id


class TestUpdateRunStatus:
    def test_update_to_running_sets_started_at(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        assert run.started_at is None

        updated = update_run_status(db_session, run, RunStatus.RUNNING)
        assert updated.status == RunStatus.RUNNING
        assert updated.started_at is not None

    def test_update_to_passed_sets_finished_at(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        update_run_status(db_session, run, RunStatus.RUNNING)
    ...
    def test_update_with_error_message(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        updated = update_run_status(
    ...
    def test_update_task_id(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        updated = update_run_status(
            db_session, run, RunStatus.RUNNING, task_id="abc-123"
        )
        assert updated.task_id == "abc-123"


class TestCancelRun:
    def test_cancel_pending_run(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        assert run.status == RunStatus.PENDING

        cancelled = cancel_run(db_session, run)
        assert cancelled.status == RunStatus.CANCELLED
        assert cancelled.finished_at is not None

    def test_cancel_running_run(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        update_run_status(
            db_session, run, RunStatus.RUNNING, task_id="task-xyz"
        )

        cancelled = cancel_run(db_session, run)
        assert cancelled.status == RunStatus.CANCELLED

    def test_cancel_already_finished_raises(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
    ...
    def test_cancel_failed_run_raises(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
    ...
    def test_cancel_cancelled_run_raises(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        update_run_status(db_session, run, RunStatus.CANCELLED)
...
class TestRetryRun:
    def test_retry_creates_new_run(self, db_session, user, repo):
        original = create_run(
            db_session,
            _run_create(repo.id, target_path="tests/retry_me", branch="feature"),
            user.id,
        )
        update_run_status(db_session, original, RunStatus.FAILED)

        new_run = retry_run(db_session, original, user.id)
    ...
    def test_retry_increments_retry_count(self, db_session, user, repo):
        run = create_run(db_session, _run_create(repo.id), user.id)
        update_run_status(db_session, run, RunStatus.ERROR)

        retry1 = retry_run(db_session, run, user.id)
        assert retry1.retry_count == 1

        update_run_status(db_session, retry1, RunStatus.ERROR)
        retry2 = retry_run(db_session, retry1, user.id)
        assert retry2.retry_count == 2

    def test_retry_preserves_variables(self, db_session, user, repo):
        original = create_run(
            db_session,
...
class TestCreateSchedule:
    def test_create_schedule_defaults(self, db_session, user, repo):
        data = _schedule_create(repo.id)
        schedule = create_schedule(db_session, data, user.id)
    ...
    def test_create_schedule_with_all_fields(self, db_session, user, repo):
...
class TestListSchedules:
    def test_list_schedules_empty(self, db_session):
        schedules = list_schedules(db_session)
        assert schedules == []

    def test_list_schedules_returns_all(self, db_session, user, repo):
        create_schedule(
...
class TestToggleSchedule:
    def test_toggle_active_to_inactive(self, db_session, user, repo):
        schedule = create_schedule(
            db_session, _schedule_create(repo.id), user.id
        )
        assert schedule.is_active is True

        toggled = toggle_schedule(db_session, schedule)
        assert toggled.is_active is False

    def test_toggle_inactive_to_active(self, db_session, user, repo):
        schedule = create_schedule(
            db_session, _schedule_create(repo.id), user.id
        )
        toggle_schedule(db_session, schedule)  # -> inactive

        toggled = toggle_schedule(db_session, schedule)
        assert toggled.is_active is True

    def test_double_toggle_returns_to_original(self, db_session, user, repo):
        schedule = create_schedule(
            db_session, _schedule_create(repo.id), user.id
        )
        original_state = schedule.is_active

        toggle_schedule(db_session, schedule)
        toggle_schedule(db_session, schedule)
...
class TestDeleteSchedule:
    def test_delete_schedule_removes_it(self, db_session, user, repo):
        schedule = create_schedule(
            db_session, _schedule_create(repo.id), user.id
        )
        ...
        result = get_schedule(db_session, schedule_id)
        assert result is None

    def test_delete_schedule_does_not_affect_others(self, db_session, user, repo):
        s1 = create_schedule(
        ...
        delete_schedule(db_session, s2)

        remaining = list_schedules(db_session)
