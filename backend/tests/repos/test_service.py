...
class TestListRepositories:
    ...
    async def test_list_with_repos(self, db_session, admin_user):
...
class TestGetRepository:
    async def test_get_by_id_found(self, db_session, admin_user):
        repo = _make_repo(admin_user.id, name="find-me")
        db_session.add(repo)
        await db_session.flush()
        await db_session.refresh(repo)

        result = await get_repository(db_session, repo.id)
        assert result is not None
        assert result.id == repo.id
        assert result.name == "find-me"

    async def test_get_by_id_not_found(self, db_session):
        result = await get_repository(db_session, 99999)
        assert result is None

    async def test_get_by_name_found(self, db_session, admin_user):
        repo = _make_repo(admin_user.id, name="named-repo")
        db_session.add(repo)
        await db_session.flush()

        result = await get_repository_by_name(db_session, "named-repo")
        assert result is not None
        assert result.name == "named-repo"

    async def test_get_by_name_not_found(self, db_session):
        result = await get_repository_by_name(db_session, "nonexistent")
        assert result is None


class TestCreateRepository:
    async def test_create(self, db_session, admin_user):
        data = RepoCreate(
            name="new-repo",
        ...
        repo = await create_repository(db_session, data, admin_user.id)
    ...
    async def test_create_with_defaults(self, db_session, admin_user):
        data = RepoCreate(
            name="default-repo",
...
class TestUpdateRepository:
    async def test_update_partial(self, db_session, admin_user):
        ...
        updated = await update_repository(db_session, repo, data)
    ...
    async def test_update_name(self, db_session, admin_user):
    ...
    async def test_update_no_fields(self, db_session, admin_user):
...
class TestDeleteRepository:
    async def test_delete(self, db_session, admin_user):
        ...
        await delete_repository(db_session, repo)

        result = await get_repository(db_session, repo_id)
        assert result is None
