...
// Story: Flow Editor — Verification & Hardening. The RF text parse/serialize
// path lives in one pure, unit-tested module (round-trip fidelity contract).
// RobotEditor delegates to it instead of carrying duplicate logic that drifts.
import {
  STEP_TYPES,
  VAR_SCOPES,
  LOOP_FLAVORS,
...
// Step types, RF interfaces, constants and makeStep now live in
// `@/components/editor/robotTextIO` (imported above) — single source of truth.
...
// escape + inline-comment + continuation round-trip fixes). See that module.

// --- Main Parser (delegates to robotTextIO) ---
function parseRobotToForm(content: string): boolean {
  try {
    setupSteps.clear() // Clear cached virtual steps
    const parsed = parseRobotText(content)
    form.settings = parsed.settings
    form.variables = parsed.variables
    form.testCases = parsed.testCases
    form.keywords = parsed.keywords
    form.preambleLines = parsed.preambleLines
    form.trailingComments = parsed.trailingComments
...
// --- Main Serializer (delegates to robotTextIO) ---
function serializeFormToRobot(): string {
  return serializeRobotForm(form, { isResource: isResource.value })
}
